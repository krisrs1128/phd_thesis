...
clustering trees. It is designed to facilitate inspection of subtree
centroids, a useful follow-up analysis of hierarchical clustering results, but
which, in the absence of supplementary tools, can be complicated to implement
and difficult to visually process. Indeed, after clustering a collection of
samples, it is worthwhile to characterize resulting clusters, rather than
stopping at the fact that certain samples were clustered together. The two most
common approaches to this characterization step are
\begin{itemize}
\item Centroids: Average quantitative features within different clusters.
  Similarly, look at cross-tabulations of categorical features across clusters.
\item Heatmaps: Produce a heatmap of the raw data, with samples sorted according
  to leaf positions in the hierarchical clustering.
\end{itemize}
...
over one node, then layer on new views associated with hovering over other
nodes. This view can clarify which features are responsible for differences
between subtrees.
...
the number of the hovered-over node's descendants with that property.
...
\ref{fig:centroidview_antibiotic_overview}. Each row corresponds to one species,
and each column is a sample. Samples are first grouped by person, then are
sorted by time. The intensity of a cell in the heatmap reflects the abundance of
that species in that sample. The hierarchical clustering tree is printed on the
left. Subtree centroids are given in the panels along the top right, with one
panel per subject and one line per subtree. Taxonomic breakdowns appear in the
bottom right. Different colors distinguish different subtrees. 
